...
def get_user_info(username="ehennenh"):
    ...
    try:
        # Try multiple authentication methods to handle MD4 issues
        server = Server(ldap_server, get_info=ALL)
        conn = None
        
        # Method 1: Try NTLM first
        try:
            ...
            print(f"✗ NTLM failed: {ntlm_error}")
            
            # Method 2: Try Simple bind (usually works)
            try:
                ...
                print(f"✗ Simple failed: {simple_error}")
                
                # Method 3: Try with just username (no domain prefix)
                try:
                    print("Trying Simple auth with username only...")
                    conn = Connection(
                        server, 
                        user=admin_username,  # Just username, no domain
                        password=admin_password, 
                        authentication=SIMPLE, 
                        auto_bind=True
                    )
                    print("✓ Simple authentication (username only) successful")
                except Exception as final_error:
                    print(f"✗ All authentication methods failed: {final_error}")
                    raise Exception("Could not authenticate with any method")
